...
export async function updateHistoryChart() {
    if (historyChart) {
        ...
        axios.get(BACKEND_URL + "/last-hb?uid=" + localStorage.getItem("uid"), {
            ...
            let difference = "";
            let oldValue = parseFloat(sessionStorage.getItem("hb"));
            let newValue = parseFloat(res.data.value);
        
            if (!isNaN(oldValue) && !isNaN(newValue)) {
                let diff = Math.floor(oldValue - newValue);
            ...
            sessionStorage.setItem("hb", newValue);  // Wichtig: neuen Wert speichern
